...
export const Updatepage = () => {
  ...
  const updateHandle = async (e) => {
    e.preventDefault();
    const res = await axios.put(`http://localhost:8000/api/update/${id}`, edit);
    navigate('/');
      ...
      <div className=' h-screen w-screen flex justify-center items-center'>
        <div className=' w-[70%]'>
          <div className='flex flex-col justify-end items-center gap-2'>
            ...
            <button onClick={() => navigate('/')} className=' bg-green-500 hover:bg-green-600 active:scale-95 duration-100 text-white rounded-xl w-[20%] px-2 py-1 text-sm'>
              Back
            </button>
          </div>
